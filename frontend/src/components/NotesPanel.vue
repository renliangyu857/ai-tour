...
watch(() => props.open, async (val) => {
  if (val && props.initialContent) {
    const note = notesStore.createNote(props.initialContent, props.initialContent)
    await nextTick()
    selectNote(note.id)
...
.nl-search:focus { border-color: #B8923A; }
...
.nl-item.active { background: rgba(28, 56, 41, 0.06); }
...
.nl-tool-btn:hover { border-color: #1C3829; color: #1C3829; }
.nl-tool-btn.active { background: rgba(28, 56, 41, 0.07); border-color: #1C3829; color: #1C3829; }
...
.nl-welcome-btn:hover { background: #2D4F3C; }
