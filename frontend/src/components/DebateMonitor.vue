<template>
  <div v-if="debate.active" class="debate-monitor">
    ...
    <!-- Vote tally bars -->
    <div v-if="Object.keys(debate.voteTally).length > 0" class="debate-votes">
      <div
        v-for="(count, vote) in debate.voteTally"
        :key="vote"
        class="vote-bar"
      >
...
function barWidth(count: number): string {
  const max = Math.max(...Object.values(props.debate.voteTally || {}), 1)
