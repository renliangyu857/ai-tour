<template>
  ...
  >
    <div class="agent-initial" :class="statusClass">{{ initial }}</div>
...
const name = computed(() => props.agent.info.displayName)
...
// Use first letter of display name as monogram
const initial = computed(() => {
  const n = props.agent.info.displayName
...
.agent-card.running {
  ...
  background: linear-gradient(90deg, rgba(28,56,41,0.04) 0%, rgba(28,56,41,0.01) 40%, transparent 100%);
}

.agent-card.completed {
  border-left-color: var(--forest-400);
  background: linear-gradient(90deg, rgba(61,112,96,0.06) 0%, transparent 100%);
}
...
.agent-card.debating {
  ...
  background: linear-gradient(90deg, rgba(184,146,58,0.08) 0%, transparent 100%);
}
