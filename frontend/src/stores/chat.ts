...
export const useChatStore = defineStore('chat', () => {
  ...
  async function sendMessage(
    ...
    try {
      if (useStream.value) {
        await sendMessageStream(sessionId, question, cities, preferences, assistantMsg)
      } else {
        await sendMessageNormal(sessionId, question, cities, preferences, assistantMsg)
      }
    } catch (error: any) {
  ...
  async function regenerateMessage(messageId: string) {
    ...
    try {
      if (useStream.value) {
        await sendMessageStream(session.id, msg.question, msg.cities ?? [], msg.preferences, msg)
      } else {
        await sendMessageNormal(session.id, msg.question, msg.cities ?? [], msg.preferences, msg)
      }
    } catch (error: any) {
  ...
  async function sendMessageNormal(
    ...
    const response = await chatApi.chat({ sessionId, cities, question, preferences, stream: false })
  ...
  async function sendMessageStream(
    ...
    const generator = chatApi.chatStream({ sessionId, cities, question, preferences, stream: true })
    assistantMsg.loading = false
