...
import com.tourism.rag.agent.provider.*;
...
public class ItineraryAgentService {
    ...
    public ItineraryResponse generate(ItineraryRequest req, Long userId) {
        ...
        try {
            ...
            Future<List<WeatherInfo>> weatherFuture = exec.submit(
                    () -> callWeather(req.getCityCode(), cityName, req.getStartDate(), req.getEndDate(), toolLogs));

            // POI（全程用同一组，每天从中挑选）
            Future<List<PoiInfo>> poiFuture = exec.submit(
                    () -> callPOI(req.getCityCode(), cityName, req.getPreferences(), toolLogs));
            ...
            try {
                ...
                log.warn("[Agent] 工具调用超时，降级为 Mock");
                allWeather = mockWeather.getWeather(req.getCityCode(), cityName, req.getStartDate(), req.getEndDate());
                allPois    = mockMap.searchPOI(req.getCityCode(), cityName,
                        List.of("景点"), req.getPreferences(), 8);
            } finally {
                exec.shutdown();
            }
            ...
            double[] cityCenter = mockMap.getCityCenter(req.getCityCode());
            ...
            for (int i = 0; i < dates.size(); i++) {
                ...
                // 取当日天气（若无则 Mock）
                WeatherInfo weather = weatherByDate.getOrDefault(dateStr,
                        mockWeather.getWeather(req.getCityCode(), cityName, dateStr, dateStr).get(0));
                ...
                // 户外 vs 室内分组
                List<PoiInfo> outdoorPois = dayPois.stream().filter(p -> !p.isIndoorVenue()).collect(Collectors.toList());
                List<PoiInfo> indoorPois  = dayPois.stream().filter(PoiInfo::isIndoorVenue).collect(Collectors.toList());
                if (indoorPois.isEmpty()) {
                    indoorPois = allPois.stream().filter(PoiInfo::isIndoorVenue).limit(2).collect(Collectors.toList());
                }

                // 路线优化
                List<PoiInfo> mainPois  = weather.isOutdoorFriendly() ? dayPois : indoorPois;
                List<PoiInfo> altPois   = weather.isOutdoorFriendly() ? indoorPois : outdoorPois;
                RouteInfo mainRoute = callRoute(mainPois, cityCenter[0], cityCenter[1],
                        req.getTransportMode(), toolLogs);
                RouteInfo altRoute  = callRoute(altPois,  cityCenter[0], cityCenter[1],
                        req.getTransportMode(), toolLogs);
                ...
                List<FoodRecommendation> foods = callFood(
                        req.getCityCode(), cityName, refLat, refLng, req.getPreferences(), toolLogs);
    ...
    private List<WeatherInfo> callWeather(String cityCode, String cityName,
        ...
        try {
            List<WeatherInfo> real = gaodeWeather.getWeather(cityCode, cityName, start, end);
            if (!real.isEmpty()) {
                provider = "gaode_weather";
                result = real;
            } else {
                fallback = true;
                result = mockWeather.getWeather(cityCode, cityName, start, end);
            }
        } catch (Exception e) {
            fallback = true;
            error = e.getMessage();
            result = mockWeather.getWeather(cityCode, cityName, start, end);
            log.warn("[Agent][getWeather] 降级到 Mock：{}", e.getMessage());
        }

        logs.add(ToolCallLog.builder()
                .toolName("getWeather").provider(provider)
                .startTime(LocalDateTime.now().format(ISO_FMT))
                .durationMs(System.currentTimeMillis() - t)
                .success(true).usedFallback(fallback).errorMessage(error).build());
        return result;
    }

    private List<PoiInfo> callPOI(String cityCode, String cityName,
        ...
        try {
            List<PoiInfo> real = gaodeMap.searchPOI(cityCode, cityName, List.of("景点"), preferences, 12);
            if (!real.isEmpty()) {
                provider = "gaode_api";
                result = real;
            } else {
                fallback = true;
                result = mockMap.searchPOI(cityCode, cityName, List.of("景点"), preferences, 12);
            }
        } catch (Exception e) {
            fallback = true;
            error = e.getMessage();
            result = mockMap.searchPOI(cityCode, cityName, List.of("景点"), preferences, 12);
            log.warn("[Agent][searchPOI] 降级到 Mock：{}", e.getMessage());
        }

        logs.add(ToolCallLog.builder()
                .toolName("searchPOI").provider(provider)
                .startTime(LocalDateTime.now().format(ISO_FMT))
                .durationMs(System.currentTimeMillis() - t)
                .success(true).usedFallback(fallback).errorMessage(error).build());
        return result;
    ...
    private RouteInfo callRoute(List<PoiInfo> pois, double lat, double lng,
                                 String transportMode, List<ToolCallLog> logs) {
        long t = System.currentTimeMillis();
        RouteInfo result = mockMap.planRoute(pois, lat, lng, transportMode);
        logs.add(ToolCallLog.builder()
                .toolName("planRoute").provider("nearest_neighbor")
                .startTime(LocalDateTime.now().format(ISO_FMT))
                .durationMs(System.currentTimeMillis() - t)
    ...
    private List<FoodRecommendation> callFood(String cityCode, String cityName,
        ...
        boolean fallback = false;
        String error = null;
        List<FoodRecommendation> result;

        try {
            List<FoodRecommendation> real = gaodeFood.recommendFood(
                    cityCode, cityName, lat, lng, "lunch", preferences, foodMinRating, foodMaxResults);
            if (!real.isEmpty()) {
                provider = "gaode_api";
                result = real;
            } else {
                fallback = true;
                result = mockFood.recommendFood(cityCode, cityName, lat, lng,
                        "lunch", preferences, foodMinRating, foodMaxResults);
            }
        } catch (Exception e) {
            fallback = true;
            error = e.getMessage();
            result = mockFood.recommendFood(cityCode, cityName, lat, lng,
                    "lunch", preferences, foodMinRating, foodMaxResults);
            log.warn("[Agent][recommendFood] 降级到 Mock：{}", e.getMessage());
        }

        logs.add(ToolCallLog.builder()
                .toolName("recommendFood").provider(provider)
                .startTime(LocalDateTime.now().format(ISO_FMT))
                .durationMs(System.currentTimeMillis() - t)
                .success(true).usedFallback(fallback).errorMessage(error).build());
        return result;
    ...
    private String resolveCityName(String cityCode) {
        return cityRepository.findByCode(cityCode)
                .map(c -> c.getNameCn())
                .orElseGet(() -> toCityNameFallback(cityCode));
    }

    private String toCityNameFallback(String cityCode) {
    ...
    private static String buildPoiNote(PoiInfo poi, WeatherInfo weather) {
        ...
        if (!poi.getTicketPrice().contains("免费")) {
